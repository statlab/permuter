...
```{r diffs}
#rel_ratings <- lapply(1:l, function(j) {t(as.matrix(sapply(2:C, function(i) (ratings[[j]][i,] - ratings[[j]][1,]))))}) # the first row is the fragrance `r`
rel_ratings <- lapply(1:l, function(j){
  tmp <- ratings[[j]][-1,]
  for(i in 1:nrow(tmp)){
    tmp[i,] <- tmp[i,] - ratings[[j]][1,]
  }
  tmp
})
...
for (j in 1:l) {
    D[, , j] <- as.matrix(waterfalls[panelist == j, ])
}
DD <- D  # auxiliary
for (i in 1:l) {
    for (j in 1:C) {
...
for(bb in 1:B){
  for(k in 1:l){
    S_star =  1-2*rbinom((C-1),1,.5)
    rel_ratings_star[[k]] <- apply(rel_ratings[[k]], 2, function(x) x*S_star)
  }
  distr[,,bb] <- Reduce("+", rel_ratings_star)
}

partial_pvalue <- sapply(1:(C-1), function(i) sapply(1:p, function(j) t2p(tst[i,j], distr[i,j,], alternative = "greater")))
