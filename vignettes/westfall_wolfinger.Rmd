...
if (p > 2) {
    for (j in 2:(p - 1)) {
        ...
        p_ris[j] = max(t2p(obs_tippett, t_tippett, alternative = "greater"), p_ris[(j-1)])
    }
}
...
if (p > 2) {
    for (j in 2:(p - 1)) {
        ...
        p_ris[j] = max(t2p(obs_fisher, t_fisher, alternative = "greater"), p_ris[(j-1)])
    }
}
p_ris[p] <- max(p_ord[p], p_ris[p - 1])
p_ris[order(p_raw)] <- p_ris
...
if (p > 2) {
    for (j in 2:(p - 1)) {
        ...
        p_ris[j] = max(t2p(obs_liptak, t_liptak, alternative = "greater"), p_ris[(j-1)])
    }
}
p_ris[p] <- max(p_ord[p], p_ris[p - 1])
p_ris[order(p_raw)] <- p_ris
...
kable(cbind(p_ris_fisher2, p_ris_liptak2, p_ris_tippett2), col.names = c("Fisher", "Liptak", "Tippett"))
