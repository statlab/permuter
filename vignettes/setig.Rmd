...
for(bb in 2:(B+1)){
  for(s in 1:nstr){
    ...
    for(j in 1:p){
      ...
      nu = table(O,paz)
      if(dim(nu)[1]>1){nu=nu[2,]}
      D1 = sum(y[paz==1]) ;  D2 = sum(y[paz==2])
      T[bb,j,s] =  D1*sqrt(nu[2]/nu[1])-D2*sqrt(nu[1]/nu[2])
      if(T[bb,j,s]=='NaN'){T[bb,j,s] = Inf}
    }
  }
}
...
for(j in 1:p){
  P.glob.v[,j] = apply(P[,j,],1,function(x){-2*log(prod(x))})
}
```
...
for(k in 1:nstr){
  T.glob.s[,k] = apply(P[,,k],1,function(x){-2*log(prod(x))})
}
...
## Global test
```{r global}
T.glob = apply(T.glob.s,1,function(x){-2*log(prod(x))}) # In the original script, this says apply(P.glob.s,....) but P.glob.s does not exist
p.glob = t2p(T.glob)[1]
p.glob
```