...
time <- botulinum[ , "Time"]
covariates <- botulinum %>% select(-paz, -tempo, -Time, -farmaco)
...
tst <- rep(0, p)
for (t in 1:3) {
    group1 <- D[[t]][treatment == "botox", ]
    group0 <- D[[t]][treatment == "sol_fisio", ]
    for (j in 1:p) {
      nu <- count_nas(D[[t]][, j], treatment)
...
for (b in 1:B) {
  ...
  for (t in 1:3) {
    group1 <- D_star[[t]][treatment == "botox", ]
    group0 <- D_star[[t]][treatment == "sol_fisio", ]
    for (j in 1:p) {
      nu <- count_nas(D_star[[t]][, j], treatment)
      omega <- sqrt(nu[2]/nu[1])
