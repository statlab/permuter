...
```{r alternatives}
colnames(botulinum)
# greater_than <- c("ME", "Mas", "Maf", "Mp", "Mll", "Mlr", "E", "T") # These don't match the column names in botulinum!!
...
for(b in 1:B){
  ...
  for(j in 1:p){
    for(t in 1:3){
      ...
      omega <- sqrt(nu[2]/nu[1])
      distr[b,j] <- distr[b,j] + omega*(mean(group1[,j], na.rm=TRUE) - mean(group0[,j], na.rm=TRUE)) 
      }
  }
}
```
...
############################################################################## NEW
# Take differences over time for individuals who received botox
D_new <- list()
for(t in 1:4){
...
for (j in 1:p) {
    ...
    for (t in 1:3) {
        ...
        T[1, j] <- T[1, j] + (S[t]*sqrt(sum(nu[-t])/nu[t]) - (sum(S) - S[t])*sqrt(sum(nu[-t])/nu[t]))^2
...
for (bb in 2:(B + 1)) {
    ...
    for (j in 1:p) {
        ...
        for (t in 1:3) {
            ...
            T[bb, j] <- T[bb, j] +(S[t]*sqrt(sum(nu[-t])/nu[t]) - (sum(S) - S[t])*sqrt(sum(nu[-t])/nu[t]))^2
...
for (j in 1:p) {
    ...
    for (t in 1:3) {
        ...
        #tst[j] <- tst[j] + (sum(nu[-t])/nu[t])*(2*S[t]-sum(S))^2 # Equivalent, fewer calculations
    }
} 
...
for (bb in 1:B) {
#    time_ints <- sample(time_ints)  # permute the times: under the null, the distributions for each are equal
    ...
    for (j in 1:p) {
      ...
      S <- apply(Y, 2, sum, na.rm=TRUE)
      for (t in 1:3) {
        distr[bb, j] <- distr[bb, j] + (S[t]*sqrt(sum(nu[-t])/nu[t]) - (sum(S) - S[t])*sqrt(sum(nu[-t])/nu[t]))^2 #+ (sum(nu[-t])/nu[t])*(2*S[t]-sum(S))^2 # Equivalent, fewer calculations       
        }
    }
}
