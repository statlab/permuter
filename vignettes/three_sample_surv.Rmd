...
for(s in 1:str){
  ...
  for(nn in 1:3){ 
    ni1[nn,] = apply(NU,3,function(x){sqrt( (sum(x[,1])-x[nn,1])/x[nn,1] )})
    S1[nn,] = apply(D,2,function(x){sum(x[g.str==(nn-1)])})
    S2[nn,] = apply(D,2,function(x){sum(x) - sum(x[g.str==(nn-1)])}  )
    }
  ni2 = 1/ni1
...
for(s in 1:str){
  ...
  plot(survfit(Surv(D$time,D$status)~g.str),col=c(2,3,4),xlab="Time",ylab="S(t)",conf.int=FALSE)
