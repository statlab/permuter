...
The array T contains has dimension (B + 1) × 1 and contains the null distribution of the test statistic. The first value is the observed value. In order to obtain a p-value we need to load the function t2p_old, that returns an array of p- values from an array of permutation values of the test statistic (i.e. it compares each element of T with the whole distribution according with the rule large is significant).
