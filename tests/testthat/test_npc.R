...
test_that("p-values from toy example", {
  ...
  expect_equal(npc(pval, distr, alternatives = c("less","greater","less","greater","two-sided")), 0.4)
})

context("Fisher")
test_that("Fisher combining function", {
  pval <- seq(0.05, 0.9, length.out = 5)
  expect_equal(fisher(pval), 11.11546, tolerance = 1e-5)
...
test_that("Liptak combining function", {
  pval <- seq(0.05, 0.9, length.out = 5)
  expect_equal(liptak(pval), 0.5728894, tolerance = 1e-5)
