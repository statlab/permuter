...
CI_mean <- function(x, y = NULL, reps=1000, side = "both", alpha = 0.05){
  ...
  if(!(side %in% c("upper", "lower", "both"))){ stop("side must be one of 'upper', 'lower', or 'both'")}  
  if(side == "both"){
  ...
  d_prev <- observed; d_next <- observed; incr <- diff(range(c(x,y)))/2
  ...
  d <- uniroot(function(z) {shift_permtest(z)-alpha}, lower = l_int, upper = u_int, tol = alpha/reps)
  return(d$root)
}